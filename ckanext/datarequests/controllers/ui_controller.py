...
class DataRequestsUI(base.BaseController):
    ...
    def vote(self, id):
        try:
            context = self._get_context()
            data_dict_vote = {'datarequest_id': id}
            tk.check_access(constants.DATAREQUEST_VOTE, context, data_dict_vote)
            data_dict_datarequest_show = {'id': id}
            # Raises 404 Not Found if the data request does not exist
            c.datarequest = tk.get_action(constants.DATAREQUEST_SHOW)(context, data_dict_datarequest_show)
            vote = request.POST.get('vote', '')
            vote_id = request.POST.get('vote-id', '')
            if request.POST:
                action = constants.DATAREQUEST_VOTE
                action_text = 'vote'

                try:
                    vote_data_dict = {'datarequest_id': id, 'vote': vote, 'id': vote_id}
                    updated_vote = tk.get_action(action)(context, cvote_data_dict)
                except tk.NotAuthorized as e:
                    log.warn(e)
                    tk.abort(403, tk._('You are not authorized to %s' % action_text))
                except tk.ValidationError as e:
            ...
            log.warn(e)
            tk.abort(403, tk._('You are not authorized to upvote/downvote Data Request %s'
                               % id))
